...
public class AggiungiAlCarrello extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//controllo se il prodotto esiste e la quantità è positiva
		...
		//controllo quantità
		if(prodotto.getQuantit�() < quantita) {//Quantità non disponibile

			//verifica ajax
			if(x!=null) {//ajax
				if(x.equalsIgnoreCase("XMLHttpRequest"))
					response.getWriter().write(""+prodotto.getQuantit�());
			}else {//richiesta diretta
				sendError(response);
				return;
			}

		}else {//la quantità va bene
			//TRUE
	...
	private void verificaProdotto(String email, ProdottoBean prodotto) {
		...
		if(c != null) {
			double prezzo = prodotto.getPrezzo()*c.getQuantit�();
	...
	private void salvaProdotto(String emailCliente, String idProdotto, int quantita) {
		...
		prodottoCarrello.setQuantit�(quantita);
