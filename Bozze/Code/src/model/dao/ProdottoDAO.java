...
public class ProdottoDAO {
	...
	public synchronized ProdottoBean doRetrieveByKey(String id){
		...
		try {
			...
			if(res.next())
			{
				...
				p.setQuantit�(res.getInt("quantit�"));
	...
	public synchronized void doUpdate(ProdottoBean p){
		...
		try {
			con = DriverManagerConnectionPool.getConnection();
			ps = con.prepareStatement("update PROGETTOTSW.Prodotto set urlImmagine=?,categoria=?,nome=?,prezzo=?,quantit�=?,descrizione=? where ID=?");
			ps.setString(1, p.getUrlImmagine());
			ps.setString(2, p.getCategoria());
			ps.setString(3, p.getNome());
			ps.setDouble(4, p.getPrezzo());
			ps.setInt(5, p.getQuantit�());
